"""Hospital (의료시설) data repository backed by local CSV files."""
...
_DATA_DIR = Path(__file__).parent.parent / "api_data" / "hospital"
...
_FILES: dict[str, str] = {
    ...
    "의원": "건강_의원_전처리.csv",
    "약국": "건강_약국_전처리.csv",
    "부속의료기관": "건강_부속의료기관_전처리.csv",
    "산후조리업": "건강_산후조리업_전처리.csv",
    "응급환자이송업": "건강_응급환자이송업_전처리.csv",
}
...
def _clean_phone(value: Any) -> str | None:
    ...
    if not s or s.lower() == "nan":
        return None
    # float로 읽힌 경우 소수점 제거 (예: '21234567.0' → '21234567')
    if s.endswith(".0"):
        s = s[:-2]
    return s or None
...
class HospitalRepository:
    """Lazy-loading repository for 의료시설 data (search + proximity search)."""
    ...
    def _load_df(self) -> pd.DataFrame:
        ...
        # 도로명주소 컬럼 통일
        if "도로명전체주소" in combined.columns and "도로명주소" not in combined.columns:
            combined["도로명주소"] = combined["도로명전체주소"]
        if "소재지전체주소" in combined.columns and "지번주소" not in combined.columns:
            combined["지번주소"] = combined["소재지전체주소"]
    ...
    def search(
        ...
        """사업장명/주소 키워드로 의료시설을 검색합니다. 업태구분명별 그룹핑 결과 반환. 각 그룹별로 limit_per_group개까지만 반환합니다."""
        df = self._load_df()
    ...
    def search_near(
        ...
        """좌표(위도/경도) 기준 반경 내 의료시설을 업태구분명별 그룹핑하여 거리순으로 반환합니다."""
        df = self._load_df()
...
def _row_to_dict(row: Any) -> dict[str, Any]:
    # category: 의료기관종별명 우선, 없으면 _category(파일 기반 분류) 사용
    med_type = row.get("의료기관종별명")
    if pd.notna(med_type) and str(med_type).strip():
        category = str(med_type)
    else:
    ...
    # 병원/의원 전용 필드
    specialty = row.get("진료과목내용명")
    if pd.notna(specialty) and str(specialty).strip():
        result["specialty"] = str(specialty)
    beds = row.get("병상수")
    if pd.notna(beds):
        try:
